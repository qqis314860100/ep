...
export function GovernanceOverviewPage({ onOpenTask, embedded }: { onOpenTask?: (taskId: number) => void; embedded?: boolean }) {
  ...
  return <section>
    {!embedded && <Header><div><Typography.Title level={3} style={{ margin: 0 }}>治理总览</Typography.Title><Typography.Text type="secondary">按任务查看闭环阶段、覆盖率和异常数量</Typography.Text></div></Header>}
    {!embedded && tasksQuery.isError && <Alert type="error" showIcon message="治理任务加载失败" />}
