...
export function GovernanceConfirmationPage({ taskId }: { taskId: number }) {
  ...
  const task = useQuery({ queryKey: ['governance-task', taskId], queryFn: () => getGovernanceTask(taskId) })
  const confirmationEnabled = task.data?.workflowVersion === 'CLOSED_LOOP_V1' && task.data.status === 'PENDING_CONFIRMATION'
  const confirmation = useQuery({ queryKey: ['governance-confirmation', taskId], queryFn: () => getCurrentConfirmation(taskId), enabled: confirmationEnabled })
  const executions = useQuery({ queryKey: ['governance-items', taskId], queryFn: () => getGovernanceItems(taskId), enabled: confirmationEnabled })
  ...
  if (task.data.status !== 'PENDING_CONFIRMATION') return <Alert type="info" showIcon message="当前任务不在业务确认阶段" />
  if (confirmation.isLoading || executions.isLoading) return <Spin tip="正在加载业务确认"><div style={{ height: 320 }} /></Spin>
  if (confirmation.isError || executions.isError) return <Alert type="error" showIcon message={(confirmation.error ?? executions.error)?.message ?? '业务确认加载失败'} />
  if (!confirmation.data || !current) return <Alert type="error" showIcon message="当前确认轮次没有待确认项" />
