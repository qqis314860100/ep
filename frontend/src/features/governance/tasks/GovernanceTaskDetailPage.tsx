...
import { getGovernanceEmployees, getGovernanceIssues, getGovernancePlans, getGovernanceTask, startGovernanceTask } from '../api'
...
export function GovernanceTaskDetailPage({ taskId, onBack }: { taskId: number; onBack?: () => void }) {
  ...
  return <article>
    <Header><Space align="start">{onBack && <Button aria-label="返回治理总览" icon={<ArrowLeftOutlined aria-hidden />} onClick={onBack} />}<div><Typography.Title level={3} style={{ margin: 0 }}>{task.name}</Typography.Title><Space><GovernanceStatusTag status={task.status} /><Typography.Text type="secondary">任务 #{task.id}</Typography.Text></Space></div></Space>{task.status === 'DRAFT' && <Button type="primary" icon={<PlayCircleOutlined aria-hidden />} loading={startMutation.isPending} onClick={() => startMutation.mutate()}>启动任务</Button>}</Header>
    {startMutation.error && <Alert type="error" showIcon message={startMutation.error.message} style={{ marginBottom: 16 }} />}
