...
export function GovernanceIssuePoolPage() {
  ...
  return <section>
    ...
    </Space></Filters><div>{issuesQuery.isError && <Alert type="error" showIcon message="问题池加载失败" />}<Table rowKey="id" size="small" loading={issuesQuery.isLoading} rowSelection={rowSelection} columns={columns} dataSource={issuesQuery.data ?? []} scroll={{ x: 820 }} pagination={{ pageSize: 10, showSizeChanger: false }} /></div></Layout>
