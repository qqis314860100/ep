...
export function GovernanceInventoryPage() {
  ...
  return (
    <GovernancePage>
      <GovernancePageHeader
        title="资产盘点"
        subtitle="统计资产质量与治理缺口，按旧维度或缺字段筛选问题资产"
        actions={<Button icon={<FilterOutlined />} onClick={() => form.submit()}>应用筛选</Button>}
      />

      <GovernanceMetricGrid>
        <GovernanceMetric label="资产总量" value={view?.totals.total ?? 0} />
        <GovernanceMetric label="待整理" value={view?.totals.pendingCuration ?? 0} />
        <GovernanceMetric label="已认领" value={view?.totals.claimed ?? 0} />
        <GovernanceMetric label="已标准化" value={view?.totals.standardized ?? 0} />
        <GovernanceMetric label="疑似重复" value={view?.totals.duplicateSuspects ?? 0} />
        <GovernanceMetric label="异常文件" value={view?.totals.anomalousFiles ?? 0} />
        <GovernanceMetric label="必填完整率" value={view?.rates.completeness ?? 0} unit="%" />
        <GovernanceMetric label="范围覆盖率" value={view?.rates.scopeCoverage ?? 0} unit="%" />
      ...
      {query.isError && <Alert type="error" showIcon message="盘点数据加载失败" />}
      <GovernancePanel>
        <Table
          rowKey="assetId"
          size="small"
