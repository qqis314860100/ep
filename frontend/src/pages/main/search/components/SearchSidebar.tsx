...
export function SearchSidebar(props: SearchSidebarProps) {
  ...
  return (
    <Panel aria-label="筛选条件">
      ...
      <Select allowClear placeholder="平台族" value={props.platformFamily} onChange={(value) => { props.onPlatformFamilyChange(value); props.onPlatformVariantChange(undefined) }} style={{ width: 124 }} options={platformFamilies.map((item) => ({ value: item.name, label: item.name }))} />
      <Select allowClear placeholder="平台子类" value={props.platformVariant} onChange={props.onPlatformVariantChange} disabled={!selectedPlatform} style={{ width: 132 }} options={platformVariants.map((item) => ({ value: item.name, label: item.name }))} />
      <Select allowClear placeholder="基地" value={props.base} onChange={(value) => { props.onBaseChange(value); props.onProductionLineChange(undefined) }} style={{ width: 120 }} options={bases.map((item) => ({ value: item.name, label: item.name }))} />
      <Select allowClear placeholder="拉线" value={props.productionLine} onChange={props.onProductionLineChange} disabled={!selectedBase} style={{ width: 112 }} options={productionLines.map((item) => ({ value: item.name, label: item.name }))} />
