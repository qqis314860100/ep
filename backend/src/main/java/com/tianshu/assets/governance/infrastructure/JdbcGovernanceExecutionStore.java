...
public class JdbcGovernanceExecutionStore extends JdbcGovernanceSupport implements GovernanceExecutionStore {
    ...
    @Override @Transactional public GovernanceResultVersion saveDraft(SaveDraft c) {
        ...
        if(current==null){
            ...
            saved=new GovernanceResultVersion(generatedId(key),item.id(),item.governanceRound(),1,c.field(),c.originalValueJson(),c.proposedValueJson(),c.standardVersion(),c.dictionaryVersions(),GovernanceResultStatus.DRAFT,"",c.actorUserId(),c.savedAt(),null,0);
