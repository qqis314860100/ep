...
3. 本轮只完成一个 Task。先做该 Task 的最小相关验证，再按 AGENTS.md 扩大必要验证。
4. 使用 rtk 执行 Git、Maven、pnpm、构建和测试；前端只使用 pnpm。不要连接生产数据库。
5. 验证失败时停止，不提交，不继续其他 Task，并在最后消息中说明失败原因。
6. 验证通过后更新 data-governance-progress.md：勾选完成项、记录验证结果、把下一 Task 移到“下一任务”；提交号以 Git 历史为准。
7. 将本轮 Task 的代码和进度记录合并为且仅为一个符合仓库规则的中文 Conventional Commit；不要提交生成物或无关文件。
8. 提交后确认工作树干净。只有 Task 17 已完成且最终验证通过时，最后消息才可包含独占一行的 <promise>COMPLETE</promise>。
