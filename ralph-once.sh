...
PROGRESS_FILE="data-governance-progress.md"
...
if grep -Fxq -- '- [x] 所有已编排任务完成' "${PROGRESS_FILE}"; then
  printf '%s\n' '<promise>COMPLETE</promise>' | tee "${LAST_MESSAGE_FILE}"
  echo "Ralph stopped: all orchestrated tasks are complete."
  exit 0
fi
...
2. 所有已勾选任务均已完成并已提交，不得重复实现、改写或重新评审。
...
10. 提交后确认工作树干净。只有进度文件中“所有已编排任务完成”已勾选且最终验证通过时，最后消息才可包含独占一行的 <promise>COMPLETE</promise>。
