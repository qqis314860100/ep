...
> 日期：2026-09-08　状态：已完成（2026-09-08，T1~T7 全部交付；完成记录见文末）　关联页面：上传页 / 资料检索 / 资产详情 / 文档中心
...
| 项 | 结果 |
| --- | --- |
| 库结构补齐 | `scripts/db/prepare_local_db.sh`（MySQL-9 兼容、幂等、additive）补齐 V1_6..V1_13 与 `operation_log_ext`；原数据计数不变，重跑全跳过 |
| local 启动 | local profile 可启动：新增 `JdbcOperationLogStore`（审计落库）+ 系统用户仓储放行 local（角色管理仍为占位实现） |
| 真实数据 | 27 条图纸/模型资产（含 22 条演示种子，可预览文件真实落盘 `backend/.data/files`）；页面验证：检索/图集/详情/治理总览/文档中心/我的上传无报错，PDF 预览正常 |
| 上传格式扩展 | 资产上传与文档中心统一放开 `.doc/.docx/.xls/.xlsx/.ppt/.pptx/.csv/.txt`：前端 accept/识别/图标/预览帧；后端 ZIP 内部分型（DOCX/XLSX/PPTX）、OLE（DOC/XLS/PPT）、CSV/TXT、LibreOffice 转 PDF（按格式写临时扩展名） |
| 端到端 | 上传页 staged 三种 Office/文本 → 资产详情 DOCX 预览转 PDF（application/pdf）→ 文档中心发布 DOCX 文档并预览；伪造/改名/可执行样本 422 |
| 提交 | 后端装配修复、库/种子脚本、后端格式扩展、前端存量类型债修复、前端功能 各为独立提交（含本计划文档） |
| 遗留说明 | 端到端验证在本地库留下了 1 条“联调演示”资产（id 128）与 1 篇已发布演示文档（id 1），便于人工复看 Office 预览；如需清除可直接删除对应 `sys_drawing`/`knowledge_document` 行（演示文件在 `backend/.data/files`，可一并清理） |
