...
当前治理 API 只覆盖问题资产的字段闭环和治理运营聚合。员工系统组织同步、部门资料征集、办公软件提醒和 RAGFlow 交付属于后续独立边界；接入时以平台任务、资产版本和权限为权威事实，外部系统不得维护可覆盖平台状态的副本。通知中心聚合（`GET /api/v1/notifications`）已交付为站内提醒前置雏形：实时只读聚合治理待办、任务到期/逾期与扫描失败，已读状态由前端本地维护，不改变任何治理状态。
...
- `GET /api/v1/notifications`：聚合通知列表（待整理/开放问题/异常文件待办汇总、7 天内到期与逾期治理任务、最近一次扫描失败），按创建时间倒序；已读状态由前端按通知 id 本地维护。
