...
## 5. 与通用"Vibe Coding 清单"的三处差异

1. **顺序**：清单用于锁定范围并人工核验，实现走"一条用例一次实现"的竖切；不采用"先写完所有测试再写代码"。
2. **产物位置**：不为测试策略新增 `PROJECT_SPEC.md`。活跃范围写在 `docs/plans/<date>-<slug>.md` 的 ticket 里；
   根目录新增 `*.md` 会被 `scripts/check_repo_structure.sh` 判为违规。
3. **集成测试定位**：不是"最后补个脚本"，而是真实库上的主验证手段，跨模块需求默认要进 `flow.mjs`。
