...
> **当前状态：真实链路已打通。**
> `flow.mjs` 会在本机 `tianshu` 库中动态创建本轮资产、标准、治理任务和文档，使用真实登录会话完成全流程。
> 测试 fixture 只用于上传文件内容，后端业务数据不使用内存仓储、演示种子或运行时 mock。
...
1. 生成测试 fixture（`scripts/e2e/.mock-files/`，含真实 `%PDF` 签名的最小合法 PDF）
...
- **阶段 1** 测试 fixture 上传（`POST /uploads/files`，PDF / X_T / TXT / PNG 均校验文件签名）
...
- **阶段 9** 统一检索：资产与文档同框命中
- **阶段 10** 前端冒烟（可选，需 `--frontend`；不传时 API 用例共 65 条）
...
- 全部生成物（`.mock-files/`、`.logs/`）已在 `.gitignore` 中忽略，不会误提交；fixture 不是业务 mock 数据。
